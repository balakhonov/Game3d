...
(function(window, undefined) {
	...
	jQuery
		.extend({
			...
			// scripts (optional): If true, will include scripts passed in
			// the html string
							...
							jQuery.each(tuples, function(i, tuple) {
								...
								deferred[tuple[1]](jQuery.isFunction(fn) ? function() {
									...
									if (returned && jQuery.isFunction(returned.promise)) {
										returned.promise().done(newDefer.resolve).fail(newDefer.reject).progress(
											newDefer.notify);
									} else {
				...
				for (; i < length; i ++) {
					if (resolveValues[i] && jQuery.isFunction(resolveValues[i].promise)) {
						resolveValues[i].promise().done(updateFunc(i, resolveContexts, resolveValues)).fail(
							deferred.reject).progress(updateFunc(i, progressContexts, progressValues));
					} else {
						-- remaining;
					}
				}
	...
	jQuery.support = (function() {
		...
		var support, all, a, select, opt, input, fragment, eventName, i, isSupported, clickFn, div = document
			.createElement("div");
		...
		jQuery(function() {
			var container, div, tds, marginDiv, divReset = "padding:0;margin:0;border:0;display:block;overflow:hidden;", body = document
	...
	jQuery.extend({
		...
		data: function(elem, name, data, pvt /* Internal Use Only */) {
			...
			if (! cache[id]) {
				...
				// Avoids exposing jQuery metadata on plain JS objects when the
				// object
			...
			// An object can be passed to jQuery.data instead of a key/value
			// pair; this gets
		...
		removeData: function(elem, name, pvt /* Internal Use Only */) {
			...
			if (name) {
				...
				if (thisCache) {
					...
					// Support array or space separated string names for data
					// keys
					if (! jQuery.isArray(name)) {
						...
						if (name in thisCache) {
							...
							// split the camel cased version by spaces unless a
							// key with the spaces exists
			...
			if (! pvt) {
				...
				// Don't destroy the parent cache unless the internal data
				// object
			...
			if (isNode) {
				...
				// Use delete when supported for expandos or `cache` is not a
	...
	jQuery
		.extend({
				...
				select: {
					get: function(elem) {
						var value, option, options = elem.options, index = elem.selectedIndex, one = elem.type === "select-one"
							|| index < 0, values = one ? null : [], max = one ? index + 1 : options.length, i = index < 0 ? max
							: one ? index : 0;
						...
						for (; i < max; i ++) {
							...
							// oldIE doesn't update selected after form
							// reset (#2551)
							if ((option.selected || i === index)
								&&
								// Don't return options that are
								// disabled or in a disabled optgroup
								(jQuery.support.optDisabled ? ! option.disabled : option
			...
			// Unused in 1.8, left in so attrFn-stabbers won't die; remove
			// in 1.9
			attrFn  : {},
				...
				if (value && elem.nodeType === 1) {
					...
					for (; i < attrNames.length; i ++) {
						...
						if (name) {
							...
							// Do not do this for boolean attributes (see
							// #10870)
							if (! isBool) {
				...
				tabIndex: {
					get: function(elem) {
						...
						return attributeNode && attributeNode.specified ? parseInt(attributeNode.value, 10)
							: rfocusable.test(elem.nodeName) || rclickable.test(elem.nodeName) && elem.href ? 0
							: undefined;
					}
				}
			}
	...
	jQuery.event = {
		...
		remove     : function(elem, types, handler, selector, mappedTypes) {
			...
			var t, tns, type, origType, namespaces, origCount, j, events, special, eventType, handleObj, elemData = jQuery
				.hasData(elem)
				&& jQuery._data(elem);
			...
			for (t = 0; t < types.length; t ++) {
				...
				namespaces = namespaces ? new RegExp("(^|\\.)" + namespaces.split(".").sort().join("\\.(?:.*\\.|)")
					+ "(\\.|$)") : null;
		...
		dispatch: function(event) {
			...
			var i, j, cur, ret, selMatch, matched, matches, handleObj, sel, related, handlers = ((jQuery._data(this,
				"events") || {})[event.type] || []), delegateCount = handlers.delegateCount, args = core_slice
				.call(arguments), run_all = ! event.exclusive && ! event.namespace, special = jQuery.event.special[event.type]
				|| {}, handlerQueue = [];
			...
			if (delegateCount && ! (event.button && event.type === "click")) {
				...
				for (cur = event.target; cur != this; cur = cur.parentNode || this) {
					...
					// Don't process clicks (ONLY) on disabled elements (#6911,
					// #8165, #11382, #11764)
					if (cur.disabled !== true || event.type !== "click") {
						...
						for (i = 0; i < delegateCount; i ++) {
							...
							if (selMatch[sel] === undefined) {
								selMatch[sel] = handleObj.needsContext ? jQuery(sel, this).index(cur) >= 0 : jQuery
		...
		fix: function(event) {
			...
			// properties
			var i, prop, originalEvent = event, fixHook = jQuery.event.fixHooks[event.type] || {}, copy = fixHook.props ? this.props
				.concat(fixHook.props)
				: this.props;
	...
	jQuery.Event = function(src, props) {
		...
		if (src && src.type) {
			...
			this.isDefaultPrevented = (src.defaultPrevented || src.returnValue === false || src.getPreventDefault
		...
		jQuery.event.special.submit = {
			setup: function() {
				...
				jQuery.event.add(this, "click._submit keypress._submit",
					function(e) {
						// Node name check avoids a VML-related crash in IE
						// (#9807)
	...
	(function(window, undefined) {
		...
		var cachedruns, assertGetIdNotName, Expr, getText, isXML, contains, compile, sortOrder, hasDuplicate, outermostContext,
			...
			Token = String, document = window.document, docElem = document.documentElement, dirruns = 0, done = 0, pop = [].pop, push = [].push, slice = [].slice,
		...
		// Augment a function for special use by Sizzle
			...
			createCache = function() {
				...
				return markFunction(function(key, value) {
					...
					// Object.prototype properties (see Issue #157)
		...
		// non-whitespace characters preceding the latter
			...
			rcomma = new RegExp("^" + whitespace + "*," + whitespace + "*"), rcombinators = new RegExp("^" + whitespace
		...
		// Easily-parseable/retrievable ID or TAG or CLASS selectors
			...
			matchExpr = {
				...
				"CHILD"       : new RegExp("^:(only|nth|first|last)-child(?:\\(" + whitespace + "*(even|odd|(([+-]|)(\\d*)n|)"
		...
		Sizzle.attr = function(elem, name) {
			...
			return val ? typeof elem[name] === "boolean" ? elem[name] ? name : null : val.specified ? val.value : null
				: null;
		};
		...
		Expr = Sizzle.selectors = {
			...
			preFilter: {
				...
				"CHILD": function(match) {
					 ...
					 * argument (even|odd|\d*|\d*n([+-]\d+)?|...) 3 xn-component
					 * of xn+y argument ([+-]?\d*n|) 4 sign of xn-component 5 x
					 * of xn-component 6 sign of y-component 7 y of y-component
					 */
					...
					if (match[1] === "nth") {
						...
						match[3] = + (match[3] ? match[4] + (match[5] || 1)
				...
				"ATTR": function(name, operator, check) {
					return function(elem, context) {
						...
						return operator === "=" ? result === check : operator === "!=" ? result !== check
							: operator === "^=" ? check && result.indexOf(check) === 0 : operator === "*=" ? check
							&& result.indexOf(check) > - 1 : operator === "$=" ? check
							&& result.substr(result.length - check.length) === check
							: operator === "~=" ? (" " + result + " ").indexOf(check) > - 1
							: operator === "|=" ? result === check
							|| result.substr(0, check.length + 1) === check + "-" : false;
					};
				},
				...
				"CHILD": function(type, argument, first, last) {
					...
					if (type === "nth") {
						return function(elem) {
							...
							// Incorporate the offset (or cast to NaN), then
		...
		sortOrder = docElem.compareDocumentPosition ? function(a, b) {
			...
			return (! a.compareDocumentPosition || ! b.compareDocumentPosition ? a.compareDocumentPosition : a
				.compareDocumentPosition(b) & 4) ? - 1 : 1;
		} : function(a, b) {
			...
			// top-level context(s)
				matchContext = addCombinator(function(elem) {
					...
					return (! leadingRelative && (xml || context !== outermostContext))
						|| ((checkContext = context).nodeType ? matchContext(elem, context, xml) : matchAnyContext(
						elem, context, xml));
				} ];
				...
				if ((matcher = Expr.relative[tokens[i].type])) {
					...
					if (matcher[expando]) {
						...
						return setMatcher(i > 1 && elementMatcher(matchers), i > 1
							&& tokens.slice(0, i - 1).join("").replace(rtrim, "$1"), matcher, i < j
							&& matcherFromTokens(tokens.slice(i, j)), j < len
							&& matcherFromTokens((tokens = tokens.slice(j))), j < len && tokens.join(""));
					}
		...
		function select(selector, context, results, seed, xml) {
			...
			if (! seed) {
				...
				if (match.length === 1) {
					...
					if (tokens.length > 2 && (token = tokens[0]).type === "ID" && context.nodeType === 9 && ! xml
					...
					for (i = matchExpr["POS"].test(selector) ? - 1 : tokens.length - 1; i >= 0; i --) {
						...
						if ((find = Expr.find[type])) {
							...
							// combinators
							if ((seed = find(token.matches[0].replace(rbackslash, ""), rsibling.test(tokens[0].type)
								&& context.parentNode || context, xml))) {
		...
		if (document.querySelectorAll) {
			(function() {
				...
				// qSa(:focus) reports false when true (Chrome 21), no need to
				// also add to buggyMatches since matches checks buggyQSA
				...
				// just skip matchesSelector for :active
					rbuggyMatches = [ ":active" ], matches = docElem.matchesSelector || docElem.mozMatchesSelector
				...
				// rbuggyQSA always contains :focus, so no need for a length
				// check
				...
				select = function(selector, context, results, seed, xml) {
					...
					if (! seed && ! xml && ! rbuggyQSA.test(selector)) {
						var groups, i, old = true, nid = expando, newContext = context, newSelector = context.nodeType === 9
							&& selector;
				...
				if (matches) {
					...
					// rbuggyMatches always contains :active and :focus, so no
					// need for a length check
					...
					Sizzle.matchesSelector = function(elem, expr) {
						...
						// rbuggyMatches always contains :active, so no need for
						// an existence check
	...
	jQuery.fn.extend({
		...
		is: function(selector) {
			return ! ! selector
				...
				// with two "p".
				rneedsContext.test(selector) ? jQuery(selector, this.context).index(this[0]) >= 0 : jQuery.filter(
					selector, this).length > 0 : this.filter(selector).length > 0);
		},

		closest: function(selectors, context) {
			var cur, i = 0, l = this.length, ret = [], pos = rneedsContext.test(selectors)
	...
	jQuery.each({
		...
		contents    : function(elem) {
			return jQuery.nodeName(elem, "iframe") ? elem.contentDocument || elem.contentWindow.document : jQuery
	...
	jQuery.extend({
		filter: function(expr, elems, not) {
			...
			return elems.length === 1 ? jQuery.find.matchesSelector(elems[0], expr) ? [ elems[0] ] : [] : jQuery.find
				.matches(expr, elems);
		},
	...
	var nodeNames = "abbr|article|aside|audio|bdi|canvas|data|datalist|details|figcaption|figure|footer|"
			...
			"<(?:" + nodeNames + ")[\\s/>]", "i"), rcheckableType = /^(?:checkbox|radio)$/,
	// checked="checked" or checked
		rchecked = /checked\s*(?:[^=]|=\s*.checked.)/i, rscriptType = /\/(java|ecma)script/i, rcleanScript = /^\s*<!(?:\[CDATA\[|\-\-)|[\]\-]{2}>\s*$/g, wrapMap = {
			...
			area    : [ 1, "<map>", "</map>" ],
			_default: [ 0, "", "" ]
		}, safeFragment = createSafeFragment(document), fragmentDiv = safeFragment.appendChild(document
			.createElement("div"));
	...
	jQuery.fn.extend({
		...
		replaceWith: function(value) {
			...
			return this.length ? this.pushStack(jQuery(jQuery.isFunction(value) ? value() : value), "replaceWith",
				value) : this;
		},
		...
		domManip: function(args, table, callback) {
			...
			if (this[0]) {
				...
				if (first) {
					...
					// Fragments from the fragment cache must always be cloned
					// and never used in place.
					for (iNoClone = results.cacheable || l - 1; i < l; i ++) {
						callback.call(table && jQuery.nodeName(this[i], "table") ? findOrAppend(this[i], "tbody")
							: this[i], i === iNoClone ? fragment : jQuery.clone(fragment, true, true));
					}
				}
				...
				if (scripts.length) {
					jQuery.each(scripts, function(i, elem) {
						if (elem.src) {
							...
							jQuery.globalEval((elem.text || elem.textContent || elem.innerHTML || "").replace(
								rcleanScript, ""));
						}
	...
	jQuery.buildFragment = function(args, context, scripts) {
		...
		// createDocumentFragment exception
		context = context || document;
	...
	jQuery.each({
		...
		jQuery.fn[name] = function(selector) {
			var elems, i = 0, ret = [], insert = jQuery(selector), l = insert.length, parent = this.length === 1
				&& this[0].parentNode;
		...
		.extend({
			clone: function(elem, dataAndEvents, deepDataAndEvents) {
				...
				if (jQuery.support.html5Clone || jQuery.isXMLDoc(elem)
					...
					// IE<=8 does not properly clone detached, unknown
					// element nodes
				} else {
				...
				if ((! jQuery.support.noCloneEvent || ! jQuery.support.noCloneChecked)
					...
					// IE copies events bound via attachEvent when using
					// cloneNode.
					// Calling detachEvent on the clone will also remove the
					// events
				...
				for (i = 0; (elem = elems[i]) != null; i ++) {
					...
					if (typeof elem === "string") {
						if (! rhtml.test(elem)) {
							...
							// Ensure a safe container in which to render
							// the html
							...
							// Take out of fragment container (we need a
							// fresh div each time)
				...
				if (fragment) {
					...
					handleScript = function(elem) {
						...
						if (! elem.type || rscriptType.test(elem.type)) {
							// Detach the script and store it in the scripts
							// array (if provided) or the fragment
							// Return truthy to indicate that it has been
							// handled
							return scripts ? scripts.push(elem.parentNode ? elem.parentNode.removeChild(elem)
								: elem) : fragment.appendChild(elem);
						}
					};

					for (i = 0; (elem = ret[i]) != null; i ++) {
						// Check if we're done after handling an executable
						// script
						if (! (jQuery.nodeName(elem, "script") && handleScript(elem))) {
							// Append to fragment and handle embedded
							// scripts
							fragment.appendChild(elem);
							if (typeof elem.getElementsByTagName !== "undefined") {
								// handleScript alters the DOM, so use
								// jQuery.merge to ensure snapshot iteration
								jsTags = jQuery.grep(jQuery.merge([], elem.getElementsByTagName("script")),
									handleScript);

								// Splice the scripts into ret after their
								// former ancestor and advance our index
								// beyond them
				...
				for (; (elem = elems[i]) != null; i ++) {
					...
					if (acceptData || jQuery.acceptData(elem)) {
						...
						if (data) {
							...
							// Remove cache only if it was not already
							// removed by jQuery.event.remove
							if (cache[id]) {

								delete cache[id];

								// IE does not allow us to delete expando
								// properties from nodes,
								// nor does it have a removeAttribute
								// function on Document nodes;
	...
	// https://developer.mozilla.org/en-US/docs/CSS/display
		rdisplayswap = /^(none|table(?!-c[ea]).+)/, rmargin = /^margin/, rnumsplit = new RegExp(
			"^(" + core_pnum + ")(.*)$", "i"), rnumnonpx = new RegExp("^(" + core_pnum + ")(?!px)[a-z%]+$", "i"), rrelNum = new RegExp(
			"^([-+])=(" + core_pnum + ")", "i"), elemdisplay = {
			BODY: "block"
		},
	...
	function augmentWidthOrHeight(elem, name, extra, isBorderBox) {
		...
		for (; i < 4; i += 2) {
			...
			// From this point on we use curCSS for maximum performance
			// (relevant in animations)
			if (isBorderBox) {
	...
	function getWidthOrHeight(elem, name, extra) {
		...
		// value
		var val = name === "width" ? elem.offsetWidth : elem.offsetHeight, valueIsBorderBox = true, isBorderBox = jQuery.support.boxSizing
	...
	jQuery.each([ "height", "width" ], function(i, name) {
		jQuery.cssHooks[name] = {
			...
			set: function(elem, value, extra) {
				return setPositiveNumber(elem, value, extra ? augmentWidthOrHeight(elem, name, extra,
	...
	jQuery(function() {
		if (! jQuery.support.reliableMarginRight) {
			jQuery.cssHooks.marginRight = {
				get: function(elem, computed) {
					// WebKit Bug 13343 - getComputedStyle returns wrong value
					// for margin-right
	...
	if (jQuery.expr && jQuery.expr.filters) {
		jQuery.expr.filters.hidden = function(elem) {
			return (elem.offsetWidth === 0 && elem.offsetHeight === 0)
				|| (! jQuery.support.reliableHiddenOffsets && ((elem.style && elem.style.display) || curCSS(elem,
				"display")) === "none");
		};
		...
		for (; i < length && (executeOnly || ! selection); i ++) {
			...
			if (typeof selection === "string") {
				if (! executeOnly || inspected[selection]) {
					...
					selection = inspectPrefiltersOrTransports(structure, options, originalOptions, jqXHR, selection,
						inspected);
				}
			}
		}
	...
	jQuery
		.extend({
			...
			ajaxSettings: {
				 ...
				 * password: null, cache: null, throws: false, traditional:
				 * false, headers: {},
				 */
			...
			ajax         : function(url, options) {
				...
				// It is defined here because jslint complains if it is
				// declared
				// at the end of the function (which would be more logical
				// and readable)
				function done(status, nativeStatusText, responses, headers) {
					...
					if (fireGlobals) {
						globalEventContext.trigger("ajax" + (isSuccess ? "Success" : "Error"), [ jqXHR, s,
				...
				// Add protocol if not provided (#5866: IE7 issue with
				// protocol-less urls)
				...
				if (s.crossDomain == null) {
					...
					s.crossDomain = ! ! (parts && (parts[1] !== ajaxLocParts[1] || parts[2] !== ajaxLocParts[2] || (parts[3] || (parts[1] === "http:" ? 80
				...
				if (! s.hasContent) {
					...
					// Get ifModifiedKey before adding the anti-cache
					// parameter
					ifModifiedKey = s.url;
	...
	jQuery
		...
		function(s, originalSettings, jqXHR) {
			...
			var callbackName, overwritten, responseContainer, data = s.data, url = s.url, hasCallback = s.jsonp !== false, replaceInUrl = hasCallback
				&& rjsonp.test(url), replaceInData = hasCallback && ! replaceInUrl
				&& typeof data === "string"
				&& ! (s.contentType || "").indexOf("application/x-www-form-urlencoded")
				&& rjsonp.test(data);

			// Handle iff the expected data type is "jsonp" or we
			// have a parameter to set
			if (s.dataTypes[0] === "jsonp" || replaceInUrl || replaceInData) {

				// Get callback name, remembering preexisting value
				// associated with it
				...
				// Use data converter to retrieve json after script
				// execution
				...
				jqXHR.always(function() {
					...
					// Save back as free
					if (s[callbackName]) {
						// make sure that re-using the options
						// doesn't screw things around
					...
					// Call if it was a function and we have a
					// response
		...
		"*": [ function(prop, value) {
			...
			if (parts) {
				...
				if (unit !== "px" && start) {
					...
					// trivial if it uses the same units
	...
	function Animation(elem, properties, options) {
		var result, index = 0, tweenerIndex = 0, length = animationPrefilters.length, deferred = jQuery.Deferred()
			.always(function() {
				// don't match elem in the :animated selector
				delete tick.elem;
			}), tick = function() {
			var currentTime = fxNow || createFxNow(), remaining = Math.max(0, animation.startTime + animation.duration
					- currentTime),
	...
	function defaultPrefilter(elem, props, opts) {
		var index, prop, value, length, dataShow, toggle, tween, hooks, oldfire, anim = this, style = elem.style, orig = {}, handled = [], hidden = elem.nodeType
			&& isHidden(elem);
	...
	Tween.prototype = {
		...
		run        : function(percent) {
			...
			if (this.options.duration) {
				this.pos = eased = jQuery.easing[this.easing](percent, this.options.duration * percent, 0, 1,
	...
	Tween.propHooks = {
		_default: {
			get: function(tween) {
				...
				// passing any value as a 4th parameter to .css will
				// automatically
	...
	jQuery.each([ "toggle", "show", "hide" ], function(i, name) {
		...
		jQuery.fn[name] = function(speed, easing, callback) {
			return speed == null || typeof speed === "boolean" ||
				...
				(! i && jQuery.isFunction(speed) && jQuery.isFunction(easing)) ? cssFn.apply(this, arguments) : this
		...
		.extend({
			...
			animate: function(prop, speed, easing, callback) {
				var empty = jQuery.isEmptyObject(prop), optall = jQuery.speed(speed, easing, callback), doAnimation = function() {
					// Operate on a copy of prop so per-property easing
					// won't be lost
			...
			stop   : function(type, clearQueue, gotoEnd) {
				...
				return this
					.each(function() {
						...
						// start the next in the queue if the last step
						// wasn't forced
						// timers currently will call their complete
						// callbacks, which will dequeue
	...
	jQuery.offset = {
		...
		setOffset: function(elem, options, i) {
			...
			var curElem = jQuery(elem), curOffset = curElem.offset(), curCSSTop = jQuery.css(elem, "top"), curCSSLeft = jQuery
	...
	jQuery.each({
		...
		jQuery.each({
			...
			jQuery.fn[funcName] = function(margin, value) {
				var chainable = arguments.length && (defaultExtra || typeof margin !== "boolean"), extra = defaultExtra
				...
				return jQuery.access(this, function(elem, type, value) {
					...
					if (elem.nodeType === 9) {
						...
						return Math.max(elem.body["scroll" + name], doc["scroll" + name], elem.body["offset" + name],
							doc["offset" + name], doc["client" + name]);
					}
