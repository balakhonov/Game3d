...
(function($, undefined) {
	...
	if (!$("<a>").outerWidth(1).jquery) {
		$
				.each([ "Width", "Height" ],
						function(i, name) {
							var side = name === "Width" ? [ "Left", "Right" ] : [ "Top", "Bottom" ], type = name
									.toLowerCase(), orig = {
	...
	$.fn.extend({
		disableSelection : function() {
			return this.bind(($.support.selectstart ? "selectstart" : "mousedown") + ".ui-disableSelection", function(
					event) {
...
(function($, undefined) {
			...
			if (isMethodCall) {
				this.each(function() {
					...
					if (methodValue !== instance && methodValue !== undefined) {
						returnValue = methodValue && methodValue.jquery ? returnValue.pushStack(methodValue.get())
								: methodValue;
						return false;
					}
				});
	...
	$.Widget.prototype = {
		...
		_setOption : function(key, value) {
			...
			if (key === "disabled") {
				this.widget().toggleClass(this.widgetFullName + "-disabled ui-state-disabled", !!value).attr(
						"aria-disabled", value);
				this.hoverable.removeClass("ui-state-hover");
				this.focusable.removeClass("ui-state-focus");
			}
							...
							function(event, handler) {
								function handlerProxy() {
									...
									if (!suppressDisabledCheck
											&& (instance.options.disabled === true || $(this).hasClass(
													"ui-state-disabled"))) {
										return;
									}
		...
		_trigger : function(type, event, data) {
			...
			return !($.isFunction(callback) && callback.apply(this.element[0], [ event ].concat(data)) === false || event
					.isDefaultPrevented());
		}
	};
			...
			function(method, defaultEffect) {
				$.Widget.prototype["_" + method] = function(element, options, callback) {
					...
					var hasOptions, effectName = !options ? method
							: options === true || typeof options === "number" ? defaultEffect : options.effect
									|| defaultEffect;
					options = options || {};
	...
	$.widget("ui.mouse", {
		...
		_mouseDestroy : function() {
			...
			if (this._mouseMoveDelegate) {
				$(document).unbind("mousemove." + this.widgetName, this._mouseMoveDelegate).unbind(
		...
		_mouseDown : function(event) {
			...
			elIsCancel = (typeof this.options.cancel === "string" && event.target.nodeName ? $(event.target).closest(
			...
			$(document).bind("mousemove." + this.widgetName, this._mouseMoveDelegate).bind(
					"mouseup." + this.widgetName, this._mouseUpDelegate);
		...
		_mouseDistanceMet : function(event) {
			return (Math.max(Math.abs(this._mouseDownEvent.pageX - event.pageX), Math.abs(this._mouseDownEvent.pageY
			...
			.widget(
					...
					$.ui.mouse,
					{
						...
						_mouseStart : function(event) {
							...
							// If ddmanager is used for droppables, set the
							// global draggable
							if ($.ui.ddmanager) {
							...
							// This is a relative to absolute position minus the
							// actual position calculation - only used for
							// relative positioned helper
							});
							...
							this._mouseDrag(event, true); // Execute the drag
															// once - this
															// causes the helper
															// not to be visible
															// before getting
							...
							// If the ddmanager is used for droppables, inform
							// the manager that dragging has started (see #5003)
							if ($.ui.ddmanager) {
						...
						_mouseDrag : function(event, noPropagation) {
							...
							// Call plugins and callbacks and use the resulting
							// position if something is returned
							if (!noPropagation) {
								var ui = this._uiHash();
						...
						_mouseStop : function(event) {
							...
							// if the original element is no longer in the DOM
							// don't bother to continue (see #8269)
							if (this.options.helper === "original"
						...
						_mouseUp : function(event) {
							...
							// If the ddmanager is used for droppables, inform
							// the manager that dragging has stopped (see #5003)
							if ($.ui.ddmanager) {
						...
						_getHandle : function(event) {
							return this.options.handle ? !!$(event.target).closest(
									this.element.find(this.options.handle)).length : true;
						},
						...
						_getParentOffset : function() {
							...
							// offset calculated on start, since the following
							// happened:
							// 1. The position of the helper is absolute, so
							// it's position is calculated based on the next
							// positioned parent
							// 2. The actual offset parent is a child of the
							// scroll parent, and the scroll parent isn't the
							// document, which means that
							// the scroll is included in the initial calculation
							// of the offset of the parent, and never
							...
							// since pageX/pageY includes this information
							// Ugly IE fix
						...
						_setContainment : function() {
							...
							this.containment = [
									...
									(over ? Math.max(ce.scrollWidth, ce.offsetWidth) : ce.offsetWidth)
											...
											- (parseInt(c.css("paddingRight"), 10) || 0) - this.helperProportions.width
						...
						_convertPositionTo : function(d, pos) {
							...
							return {
								top : (pos.top + // The absolute mouse
													// position
								this.offset.relative.top * mod + // Only for
																	// relative
																	// positioned
																	// nodes:
																	// Relative
								...
								left : (pos.left + // The absolute mouse
													// position
								this.offset.relative.left * mod + // Only for
																	// relative
																	// positioned
																	// nodes:
																	// Relative
						...
						_generatePosition : function(event) {
							...
							var containment, co, top, left, o = this.options, scroll = this.cssPosition === "absolute"
									&& !(this.scrollParent[0] !== document && $.contains(this.scrollParent[0],
											this.offsetParent[0])) ? this.offsetParent : this.scrollParent, pageX = event.pageX, pageY = event.pageY;
							...
							/*
							 * - Position constraining - Constrain the position
							 * to a mix of grid, containment.
							 */

							// If we are not dragging yet, we won't check for
							// options
							if (this.originalPosition) {
								if (this.containment) {
									if (this.relative_container) {
										co = this.relative_container.offset();
										containment = [ this.containment[0] + co.left, this.containment[1] + co.top,
												this.containment[2] + co.left, this.containment[3] + co.top ];
									} else {
								...
								if (o.grid) {
									// Check for grid elements set to 0 to
									// prevent divide by 0 error causing invalid
									// argument errors in IE (see ticket #6950)
									top = o.grid[1] ? this.originalPageY
											+ Math.round((pageY - this.originalPageY) / o.grid[1]) * o.grid[1]
											: this.originalPageY;
									pageY = containment ? ((top - this.offset.click.top >= containment[1] || top
											- this.offset.click.top > containment[3]) ? top : ((top
									...
									pageX = containment ? ((left - this.offset.click.left >= containment[0] || left
											- this.offset.click.left > containment[2]) ? left : ((left
											- this.offset.click.left >= containment[0]) ? left - o.grid[0] : left
											+ o.grid[0])) : left;
								}
						...
						_trigger : function(type, event, ui) {
							...
							// The absolute position has to be recalculated
							// after plugins
							if (type === "drag") {
								this.positionAbs = this._convertPositionTo("absolute");
							}
			...
			$(o.connectToSortable).each(function() {
				...
				if (sortable && !sortable.options.disabled) {
					...
					sortable.refreshPositions(); // Call the sortable's
													// refreshPositions at drag
													// start to refresh the
													// containerCache since the
													// sortable container cache
													// is used in drag and needs
													// to be up to date (this
													// will ensure it's
													// initialised as well as
			...
			$.each(inst.sortables, function() {
				...
				if (this.instance._intersectsWith(this.instance.containerCache)) {
					...
					$.each(inst.sortables, function() {
						...
						this.instance.helperProportions = inst.helperProportions;
						this.instance.offset.click = inst.offset.click;
						if (this !== thisSortable && this.instance._intersectsWith(this.instance.containerCache)
			...
			.add(
					...
					"snap",
					{
						...
						drag : function(event, ui) {
							...
							var ts, bs, ls, rs, l, r, t, b, i, first, inst = $(this).data("ui-draggable"), o = inst.options, d = o.snapTolerance, x1 = ui.offset.left, x2 = x1
									+ inst.helperProportions.width, y1 = ui.offset.top, y2 = y1
...
(function($, undefined) {
	...
	$.widget("ui.droppable", {
		...
		_drop : function(event, custom) {
			...
			this.element.find(":data(ui-droppable)").not(".ui-draggable-dragging").each(
					function() {
	...
	$.ui.intersect = function(draggable, droppable, toleranceMode) {
		...
		var draggableLeft, draggableTop, x1 = (draggable.positionAbs || draggable.position.absolute).left, x2 = x1
				+ draggable.helperProportions.width, y1 = (draggable.positionAbs || draggable.position.absolute).top, y2 = y1
				+ draggable.helperProportions.height, l = droppable.offset.left, r = l + droppable.proportions.width, t = droppable.offset.top, b = t
		...
		case "intersect":
			...
			t < y1 + (draggable.helperProportions.height / 2) && // Bottom
																	// Half
	...
	$.ui.ddmanager = {
		...
		prepareOffsets : function(t, event) {
			...
			var i, j, m = $.ui.ddmanager.droppables[t.options.scope] || [], type = event ? event.type : null, // workaround
																												// for
																												// #2317
		...
		drop : function(draggable, event) {
			...
			var dropped = false;
			// Create a copy of the droppables in case the list changes during
			// the drop (#9116)
		...
		drag : function(draggable, event) {
			...
			$.each($.ui.ddmanager.droppables[draggable.options.scope] || [],
					function() {
						...
						var parentInstance, scope, parent, intersects = $.ui.intersect(draggable, this,
								this.options.tolerance), c = !intersects && this.isover ? "isout" : (intersects
					...
					$.ui.mouse,
					{
						...
						_create : function() {
							...
							if (this.element[0].nodeName.match(/canvas|textarea|input|select|button|img/i)) {
								...
								// Create a wrapper element and set the wrapper
								// to the new current internal element
								...
								// Push the actual element to our
								// proportionallyResize internal array
							...
							this._renderAxis = function(target) {
								...
								for (i in this.handles) {
									...
									// Apply pad to wrapper element, needed to
									// fix axis position (textarea, inputs,
									// scrolls)
									if (this.elementIsWrapper
										...
										padPos = [
												"padding",
												/ne|nw|n/.test(i) ? "Top" : /se|sw|s/.test(i) ? "Bottom" : /^e$/
														.test(i) ? "Right" : "Left" ].join("");
						...
						_destroy : function() {
							...
							var wrapper, _destroy = function(exp) {
								$(exp).removeClass("ui-resizable ui-resizable-disabled ui-resizable-resizing")
										.removeData("resizable").removeData("ui-resizable").unbind(".resizable").find(
						...
						_mouseDrag : function(event) {
							...
							// Put this in the mouseDrag handler since the user
							// can start pressing shift while resizing
						...
						_mouseStop : function(event) {
							...
							if (this._helper) {
								...
								left = (parseInt(that.element.css("left"), 10) + (that.position.left - that.originalPosition.left))
										|| null;
								top = (parseInt(that.element.css("top"), 10) + (that.position.top - that.originalPosition.top))
										|| null;
						...
						_updateVirtualBoundaries : function(forceAspectRatio) {
							...
							if (this._aspectRatio || forceAspectRatio) {
								// We want to create an enclosing box whose
								// aspect ration is the requested one
								// First, compute the "projected" size for each
								// dimension based on the aspect ratio and other
								// dimension
						...
						_respectSize : function(data) {
							...
							var o = this._vBoundaries, a = this.axis, ismaxw = isNumber(data.width) && o.maxWidth
									&& (o.maxWidth < data.width), ismaxh = isNumber(data.height) && o.maxHeight
									&& (o.maxHeight < data.height), isminw = isNumber(data.width) && o.minWidth
									&& (o.minWidth > data.width), isminh = isNumber(data.height) && o.minHeight
									&& (o.minHeight > data.height), dw = this.originalPosition.left
									+ this.originalSize.width, dh = this.position.top + this.size.height, cw = /sw|nw|w/
									.test(a), ch = /nw|ne|n/.test(a);
							if (isminw) {
							...
							for (i = 0; i < this._proportionallyResizeElements.length; i++) {
								...
								if (!this.borderDif) {
									...
									for (j = 0; j < borders.length; j++) {
										this.borderDif[j] = (parseInt(borders[j], 10) || 0)
	...
	$.ui.plugin
			.add(
					...
					"animate",
					{
						...
						stop : function(event) {
							var that = $(this).data("ui-resizable"), o = that.options, pr = that._proportionallyResizeElements, ista = pr.length
									&& (/textarea/i).test(pr[0].nodeName), soffseth = ista
									&& $.ui.hasScroll(pr[0], "left") /*
																		 * TODO -
																		 * jump
																		 * height
																		 */? 0
									: that.sizeDiff.height, soffsetw = ista ? 0 : that.sizeDiff.width, style = {
								width : (that.size.width - soffsetw),
								height : (that.size.height - soffseth)
							}, left = (parseInt(that.element.css("left"), 10) + (that.position.left - that.originalPosition.left))
									|| null, top = (parseInt(that.element.css("top"), 10) + (that.position.top - that.originalPosition.top))
									|| null;
							...
							that.element.animate($.extend(style, top && left ? {
								...
								step : function() {
									...
									// propagating resize, and updating values
	...
	$.ui.plugin
			.add(
					...
					"containment",
					{
						...
						start : function() {
							var element, p, co, ch, cw, width, height, that = $(this).data("ui-resizable"), o = that.options, el = that.element, oc = o.containment, ce = (oc instanceof $) ? oc
									.get(0)
						...
						resize : function(event) {
							var woset, hoset, isParent, isOffsetRelative, that = $(this).data("ui-resizable"), o = that.options, co = that.containerOffset, cp = that.position, pRatio = that._aspectRatio
									|| event.shiftKey, cop = {
								top : 0,
								left : 0
							...
							if (cp.left < (that._helper ? co.left : 0)) {
								that.size.width = that.size.width
										+ (that._helper ? (that.position.left - co.left)
							...
							woset = Math.abs((that._helper ? that.offset.left - cop.left
									...
									+ that.sizeDiff.width);
							hoset = Math.abs((that._helper ? that.offset.top - cop.top : (that.offset.top - co.top))
									+ that.sizeDiff.height);
						...
						stop : function() {
							var that = $(this).data("ui-resizable"), o = that.options, co = that.containerOffset, cop = that.containerPosition, ce = that.containerElement, helper = $(that.helper), ho = helper
									.offset(), w = helper.outerWidth() - that.sizeDiff.width, h = helper.outerHeight()
									- that.sizeDiff.height;
						...
						resize : function(event, ui) {
							...
							_alsoResize = function(exp, c) {
								$(exp)
										.each(
												function() {
													var el = $(this), start = $(this).data("ui-resizable-alsoresize"), style = {}, css = c
															&& c.length ? c
															: el.parents(ui.originalElement[0]).length ? [ "width",
																	"height" ] : [ "width", "height", "top", "left" ];
					...
					"grid",
					{
						...
						resize : function() {
							var that = $(this).data("ui-resizable"), o = that.options, cs = that.size, os = that.originalSize, op = that.originalPosition, a = that.axis, grid = typeof o.grid === "number" ? [
									o.grid, o.grid ]
									: o.grid, gridX = (grid[0] || 1), gridY = (grid[1] || 1), ox = Math
									.round((cs.width - os.width) / gridX)
									* gridX, oy = Math.round((cs.height - os.height) / gridY) * gridY, newWidth = os.width
									+ ox, newHeight = os.height + oy, isMaxWidth = o.maxWidth
									&& (o.maxWidth < newWidth), isMaxHeight = o.maxHeight && (o.maxHeight < newHeight), isMinWidth = o.minWidth
									&& (o.minWidth > newWidth), isMinHeight = o.minHeight && (o.minHeight > newHeight);
			...
			.widget(
					...
					$.ui.mouse,
					{
						...
						_create : function() {
							...
							// Let's determine if the items are being displayed
							// horizontally
							this.floating = this.items.length ? o.axis === "x" || isFloating(this.items[0].item)
									: false;
						...
						_setOption : function(key, value) {
							if (key === "disabled") {
								...
								// Don't call widget base _setOption for disable
								// as it adds ui-state-disabled class
								$.Widget.prototype._setOption.apply(this, arguments);
							}
						},
						...
						_mouseCapture : function(event, overrideHandle) {
							...
							// parents) is a actual item in this.items
						...
						_mouseStart : function(event, overrideHandle, noActivation) {
							...
							// the refreshItems call has been moved to
							// mouseCapture
							...
							// original so it's not playing any role during the
							// drag, won't cause anything bad this way
							...
							if (o.cursor && o.cursor !== "auto") { // cursor
								...
								this.storedStylesheet = $("<style>*{ cursor: " + o.cursor + " !important; }</style>")
										.appendTo(body);
							}
							...
							this._mouseDrag(event); // Execute the drag once -
													// this causes the helper
						...
						_mouseDrag : function(event) {
							...
							for (i = this.items.length - 1; i >= 0; i--) {
								...
								// Container, skip all
								// items form other containers. This works
								// because when moving
								...
								// can cause the placeholder to jitter
						...
						cancel : function() {
							...
							if (this.placeholder) {
								// $(this.placeholder[0]).remove(); would have
								// been the jQuery way - unfortunately, it
						...
						_intersectsWith : function(item) {
							...
							var x1 = this.positionAbs.left, x2 = x1 + this.helperProportions.width, y1 = this.positionAbs.top, y2 = y1
									+ this.helperProportions.height, l = item.left, r = l + item.width, t = item.top, b = t
									+ item.height, dyClick = this.offset.click.top, dxClick = this.offset.click.left, isOverElementHeight = (this.options.axis === "x")
									|| ((y1 + dyClick) > t && (y1 + dyClick) < b), isOverElementWidth = (this.options.axis === "y")
									|| ((x1 + dxClick) > l && (x1 + dxClick) < r), isOverElement = isOverElementHeight
									&& isOverElementWidth;
						...
						_intersectsWithPointer : function(item) {
							...
							var isOverElementHeight = (this.options.axis === "x")
									|| isOverAxis(this.positionAbs.top + this.offset.click.top, item.top, item.height), isOverElementWidth = (this.options.axis === "y")
									|| isOverAxis(this.positionAbs.left + this.offset.click.left, item.left, item.width), isOverElement = isOverElementHeight
									&& isOverElementWidth, verticalDirection = this._getDragVerticalDirection(), horizontalDirection = this
							...
							return this.floating ? (((horizontalDirection && horizontalDirection === "right") || verticalDirection === "down") ? 2
									: 1)
						...
						_intersectsWithSides : function(item) {
							...
							var isOverBottomHalf = isOverAxis(this.positionAbs.top + this.offset.click.top, item.top
									...
									this.positionAbs.left + this.offset.click.left, item.left + (item.width / 2),
									item.width), verticalDirection = this._getDragVerticalDirection(), horizontalDirection = this
									._getDragHorizontalDirection();
											...
											queries
													.push([
															$.isFunction(inst.options.items) ? inst.options.items
																	.call(inst.element) : $(inst.options.items,
																	inst.element).not(".ui-sortable-helper").not(
						...
						_refreshItems : function(event) {
							...
							var i, j, cur, inst, targetData, _queries, item, queriesLength, items = this.items, queries = [ [
									$.isFunction(this.options.items) ? this.options.items.call(this.element[0], event,
											{
												item : this.currentItem
											}) : $(this.options.items, this.element), this ] ], connectWith = this
									._connectWith();

							if (connectWith && this.ready) { // Shouldn't be
																// run the first
																// time through
																// due to
																// massive
							...
							for (i = queries.length - 1; i >= 0; i--) {
								...
								for (j = 0, queriesLength = _queries.length; j < queriesLength; j++) {
									...
									item.data(this.widgetName + "-item", targetData); // Data
																						// for
																						// target
																						// checking
																						// (mouse
						...
						refreshPositions : function(fast) {
							...
							// This has to be redone because due to the item
							// being moved out/into the offsetParent, the
							// offsetParent's position will change
							...
							for (i = this.items.length - 1; i >= 0; i--) {
								...
								// We ignore calculating positions of all
								// connected containers when we're not over them
						...
						_createPlaceholder : function(that) {
							...
							if (!o.placeholder || o.placeholder.constructor === String) {
								...
								o.placeholder = {
									element : function() {
										...
										var nodeName = that.currentItem[0].nodeName.toLowerCase(), element = $(
										...
										if (nodeName === "tr") {
											that.currentItem.children().each(
													function() {
														$("<td>&#160;</td>", that.document[0]).attr("colspan",
									...
									update : function(container, p) {
										...
										// 'placeholder option, we don't force
										// sizes - the class is responsible for
										// that
										// 2. The option 'forcePlaceholderSize
										// can be enabled to force it even if a
										...
										// height by itself (without styles
										// coming from a stylesheet), it
										// receives the inline height from the
										// dragged item
										if (!p.height()) {
							...
							// Update the size of the placeholder (TODO: Logic
							// to fuzzy, see line 316/317)
						...
						_contactContainers : function(event) {
							...
							for (i = this.containers.length - 1; i >= 0; i--) {
								...
								// never consider a container that's located
								...
								if (this._intersectsWith(this.containers[i].containerCache)) {
									...
									// if we've already found a container and
									// it's more "inner" than this, then
									// continue
									...
									// container doesn't intersect. trigger
									// "out" event if necessary
									if (this.containers[i].containerCache.over) {
							...
							// move the item into the container if it's not
							// there already
							if (this.containers.length === 1) {
								...
								// When entering a new container, we will find
								// the item with the least distance and append
								// our item near it
								dist = 10000;
								...
								for (j = this.items.length - 1; j >= 0; j--) {
									...
									if (floating
											&& !isOverAxis(this.positionAbs.top + this.offset.click.top,
													this.items[j].top, this.items[j].height)) {
										continue;
									}
						...
						_createHelper : function(event) {
							...
							// already
							if (!helper.parents("body").length) {
								$(o.appendTo !== "parent" ? o.appendTo : this.currentItem[0].parentNode)[0]
						...
						_convertPositionTo : function(d, pos) {
							...
							var mod = d === "absolute" ? 1 : -1, scroll = this.cssPosition === "absolute"
									&& !(this.scrollParent[0] !== document && $.contains(this.scrollParent[0],
											this.offsetParent[0])) ? this.offsetParent : this.scrollParent, scrollIsRootNode = (/(html|body)/i)
							...
							return {
								...
								this.offset.parent.top * mod - // The
																...
																// (offset +
																// border)
								((this.cssPosition === "fixed" ? -this.scrollParent.scrollTop() : (scrollIsRootNode ? 0
								...
								this.offset.parent.left * mod - // The
																...
																// (offset +
																// border)
								((this.cssPosition === "fixed" ? -this.scrollParent.scrollLeft() : scrollIsRootNode ? 0
						...
						_generatePosition : function(event) {
							...
							// 2. and the scroll parent is the document or
							// similar to the offset parent
							...
							if (this.originalPosition) { // If we are not
								...
								if (o.grid) {
									top = this.originalPageY + Math.round((pageY - this.originalPageY) / o.grid[1])
											* o.grid[1];
									pageY = this.containment ? ((top - this.offset.click.top >= this.containment[1] && top
											- this.offset.click.top <= this.containment[3]) ? top : ((top
											- this.offset.click.top >= this.containment[1]) ? top - o.grid[1] : top
											+ o.grid[1]))
											: top;

									left = this.originalPageX + Math.round((pageX - this.originalPageX) / o.grid[0])
											* o.grid[0];
									pageX = this.containment ? ((left - this.offset.click.left >= this.containment[0] && left
											- this.offset.click.left <= this.containment[2]) ? left : ((left
											- this.offset.click.left >= this.containment[0]) ? left - o.grid[0] : left
											+ o.grid[0]))
											: left;
								}
						...
						_rearrange : function(event, i, a, hardRefresh) {
							...
							// Various things done here to improve the
							// performance:
							// 1. we create a setTimeout, that calls
							// refreshPositions
							// 2. on the instance, we have a counter variable,
							// that get's higher after every append
							// 3. on the local scope, we copy the counter
							// variable, and check in the timeout, if it's still
							// the same
							// 4. this lets only the last addition to the
							// timeout stack through
							this.counter = this.counter ? ++this.counter : 1;
							var counter = this.counter;

							this._delay(function() {
								if (counter === this.counter) {
									this.refreshPositions(!hardRefresh); // Precompute
																			// after
																			// each
																			// DOM
																			// insertion,
						...
						_clear : function(event, noPropagation) {
							...
							// after the point where the placeholder has been
							// removed and
							...
							// removed (by a user), or it gets reappended (see
							// #4088)
							...
							if ((this.fromOutside
									|| this.domPosition.prev !== this.currentItem.prev().not(".ui-sortable-helper")[0] || this.domPosition.parent !== this.currentItem
									.parent()[0])
									&& !noPropagation) {
								delayedTriggers.push(function(event) {
									this._trigger("update", event, this._uiHash());
								}); // Trigger update callback if the DOM
									// position has changed
							}

							// Check if the items Container has Changed and
							// trigger appropriate
							// events.
							...
							// the jQuery way - unfortunately, it unbinds ALL
...
(function($, undefined) {
	...
	(function(jQuery, undefined) {
		...
		function stringParse(string) {
			...
			each(
					stringParsers,
					function(i, parser) {
						var parsed, match = parser.re.exec(string), values = match && parser.parse(match), spaceName = parser.space
								|| "rgba";
						...
						if (values) {
							...
							// exit each( stringParsers ) here because we
							// matched
							return false;
						}
					});
				...
				.extend(
						color.prototype,
						{
							parse : function(red, green, blue, alpha) {
								...
								// more than 1 argument specified - assume (
										...
										each(spaces, function(spaceName, space) {
											...
											each(space.props, function(key, prop) {
												...
												if (!inst[cache] && space.to) {
													...
													// if the value was null, we
													// don't need to copy it
													// if the key was alpha, we
													// don't need to copy it
													// either
							...
							transition : function(other, distance) {
								var end = color(other), spaceName = end._space(), space = spaces[spaceName], startColor = this
										.alpha() === 0 ? color("transparent") : this, start = startColor[space.cache]
										|| space.to(startColor._rgba), result = start.slice();
		...
		spaces.hsla.to = function(rgba) {
			...
			var r = rgba[0] / 255, g = rgba[1] / 255, b = rgba[2] / 255, a = rgba[3], max = Math.max(r, g, b), min = Math
					.min(r, g, b), diff = max - min, add = max + min, l = add * 0.5, h, s;
		...
		spaces.hsla.from = function(hsla) {
			...
			var h = hsla[0] / 360, s = hsla[1], l = hsla[2], a = hsla[3], q = l <= 0.5 ? l * (1 + s) : l + s - l * s, p = 2
					* l - q;

			return [ Math.round(hue2rgb(p, q, h + (1 / 3)) * 255), Math.round(hue2rgb(p, q, h) * 255),
		...
		each(spaces,
				function(spaceName, space) {
					...
					// makes red() green() blue() alpha() hue() saturation()
					// lightness()
					each(props, function(key, prop) {
						...
						color.fn[key] = function(value) {
							var vtype = jQuery.type(value), fn = (key === "alpha" ? (this._hsla ? "hsla" : "rgba")
					...
					set : function(elem, value) {
						...
						if (value !== "transparent"
							...
							if (!support.rgba && value._rgba[3] !== 1) {
								...
								while ((backgroundColor === "" || backgroundColor === "transparent") && curElem
										&& curElem.style) {
									try {
								...
								value = value
										.blend(backgroundColor && backgroundColor !== "transparent" ? backgroundColor
												: "_default");
							}
	...
	(function() {
		...
		function getElementStyles(elem) {
			var key, len, style = elem.ownerDocument.defaultView ? elem.ownerDocument.defaultView.getComputedStyle(
					elem, null) : elem.currentStyle, styles = {};
		...
		$.effects.animateClass = function(value, duration, easing, callback) {
			...
			return this
					.queue(function() {
						var animated = $(this), baseClass = animated.attr("class") || "", applyClassChange, allAnimations = o.children ? animated
								.find("*").addBack()
								: animated;

						// map the animated objects to store the original
						// styles.
						...
						// map all animated objects again - calculate new styles
						// and diff
						allAnimations = allAnimations.map(function() {
						...
						// map all animated objects again - this time collecting
						// a promise
						allAnimations = allAnimations.map(function() {
	...
	(function() {
		...
		function _normalizeArguments(effect, options, speed, callback) {
			...
			effect.duration = $.fx.off ? 0 : typeof speed === "number" ? speed
				...
				.extend({
					effect : function( /* effect, options, speed, callback */) {
						...
						if ($.fx.off || !effectMethod) {
							// delegate to the original method (e.g., .show())
							// if possible
							if (mode) {
						...
						function run(next) {
							...
							// If the element already has the correct final
							// state, delegate to
			...
			.widget(
					"ui.accordion",
					{
						...
						_createIcons : function() {
							...
							if (icons) {
								$("<span>").addClass("ui-accordion-header-icon ui-icon " + icons.header).prependTo(
										this.headers);
						...
						_destroyIcons : function() {
							this.headers.removeClass("ui-accordion-icons").children(".ui-accordion-header-icon")
									.remove();
						},
						...
						_destroy : function() {
							...
							this.headers
									...
									.removeAttr("role").removeAttr("aria-selected").removeAttr("aria-controls")
						...
						_processPanels : function() {
							...
							this.headers.next().addClass(
									"ui-accordion-content ui-helper-reset ui-widget-content ui-corner-bottom").filter(
									":not(.ui-accordion-content-active)").hide();
						},

						_refresh : function() {
							var maxHeight, options = this.options, heightStyle = options.heightStyle, parent = this.element
									.parent(), accordionId = this.accordionId = "ui-accordion-"
						...
						_eventHandler : function(event) {
							var options = this.options, active = this.active, clicked = $(event.currentTarget), clickedIsActive = clicked[0] === active[0], collapsing = clickedIsActive
									&& options.collapsible, toShow = collapsing ? $() : clicked.next(), toHide = active
									.next(), eventData = {
								oldHeader : active,
								oldPanel : toHide,
							...
							if (options.icons) {
								active.children(".ui-accordion-header-icon").removeClass(options.icons.activeHeader)
							...
							if (!clickedIsActive) {
								...
								if (options.icons) {
									clicked.children(".ui-accordion-header-icon").removeClass(options.icons.header)
						...
						_animate : function(toShow, toHide, data) {
							var total, easing, duration, that = this, adjust = 0, down = toShow.length
									&& (!toHide.length || (toShow.index() < toHide.index())), animate = this.options.animate
						...
						_toggleComplete : function(data) {
							...
							toHide.removeClass("ui-accordion-content-active").prev().removeClass("ui-corner-top")
...
(function($, undefined) {
	...
	$.widget("ui.autocomplete", {
		...
		_create : function() {
			...
			// search term. #7799
			var suppressKeyPress, suppressKeyPressRepeat, suppressInput, nodeName = this.element[0].nodeName
					.toLowerCase(), isTextarea = nodeName === "textarea", isInput = nodeName === "input";
			...
			this._on(this.menu.element, {
				mousedown : function(event) {
					...
					// afterward
					// so we have to track the next mousedown and close the menu
					// if
					...
					if (!$(event.target).closest(".ui-menu-item").length) {
						this._delay(function() {
							...
							this.document.one("mousedown", function(event) {
								if (event.target !== that.element[0] && event.target !== menuElement
				...
				menufocus : function(event, ui) {
					...
					if (false !== this._trigger("focus", event, {
						...
						// Normally the input is populated with the item's value
						// as the
						// menu is navigated, causing screen readers to notice a
						// change and
						// announce the item. Since the focus event was
						// canceled, this doesn't
						// happen, so we update the live region so that screen
						// readers can
			...
			.widget(
					"ui.button",
					{
						...
						_create : function() {
							...
							var that = this, options = this.options, toggleButton = this.type === "checkbox"
									|| this.type === "radio", activeClass = !toggleButton ? "ui-state-active" : "", focusClass = "ui-state-focus";

							if (options.label === null) {
								options.label = (this.type === "input" ? this.buttonElement.val() : this.buttonElement
										.html());
							}

							this._hoverable(this.buttonElement);

							this.buttonElement.addClass(baseClasses).attr("role", "button").bind(
							...
							if (toggleButton) {
								...
								// if mouse moves between mousedown and mouseup
								// (drag) set clickDragged flag
								...
								if (this.buttonElement.is("a")) {
									this.buttonElement.keyup(function(event) {
										if (event.keyCode === $.ui.keyCode.SPACE) {
											...
											// correctly (just as 2nd argument
											// doesn't work)
											$(this).click();
										}
									});
							...
							// TODO: pull out $.Widget's handling for the
							// disabled option into
							// $.Widget.prototype._setOptionDisabled so it's
							// easy to proxy and can
						...
						_determineButtonType : function() {
							...
							if (this.type === "checkbox" || this.type === "radio") {
								// we don't search against the document in case
								// the element
						...
						_destroy : function() {
							...
							this.buttonElement.removeClass(baseClasses + " " + stateClasses + " " + typeClasses)
									.removeAttr("role").removeAttr("aria-pressed").html(
						...
						refresh : function() {
							// See #8237 & #8828
							var isDisabled = this.element.is("input, button") ? this.element.is(":disabled")
							...
							if (this.type === "radio") {
								radioGroup(this.element[0]).each(
										function() {
											if ($(this).is(":checked")) {
												$(this).button("widget").addClass("ui-state-active").attr(
														"aria-pressed", "true");
											} else {
												$(this).button("widget").removeClass("ui-state-active").attr(
														"aria-pressed", "false");
											}
										});
						...
						_resetButton : function() {
							...
							var buttonElement = this.buttonElement.removeClass(typeClasses), buttonText = $(
									"<span></span>", this.document[0]).addClass("ui-button-text").html(
									this.options.label).appendTo(buttonElement.empty()).text(), icons = this.options.icons, multipleIcons = icons.primary
									&& icons.secondary, buttonClasses = [];
			...
			.widget(
					"ui.buttonset",
					{
						...
						refresh : function() {
							...
							this.buttons = this.element.find(this.options.items).filter(":ui-button").button("refresh")
									.end().not(":ui-button").button().end().map(function() {
										return $(this).button("widget")[0];
									}).removeClass("ui-corner-all ui-corner-left ui-corner-right").filter(":first")
									.addClass(rtl ? "ui-corner-right" : "ui-corner-left").end().filter(":last")
									.addClass(rtl ? "ui-corner-left" : "ui-corner-right").end().end();
						},
...
(function($, undefined) {
	...
	function Datepicker() {
		...
		this.regional[""] = { // Default regional settings
			...
			monthNames : [ "January", "February", "March", "April", "May", "June", "July", "August", "September",
					"October", "November", "December" ], // Names of months
															// for drop-down and
															// formatting
		...
		this._defaults = { // Global defaults for all the date picker instances
			...
			selectOtherMonths : false, // True to allow selection of dates in
										// other months, false for unselectable
	...
	$
			.extend(
					Datepicker.prototype,
					{
						...
						// Keep track of the maximum number of rows displayed
						// (see #7043)
						maxRows : 4,

						// TODO rename to "widget" when switching to widget
						// factory
						...
						_connectDatepicker : function(target, inst) {
							...
							// If disabled option is true, disable the
							// datepicker once it has been attached to the input
							// (see ticket #5665)
						...
						_attachments : function(input, inst) {
							var showOn, buttonText, buttonImage, appendText = this._get(inst, "appendText"), isRTL = this
									._get(inst, "isRTL");
						...
						_autoSize : function(inst) {
							if (this._get(inst, "autoSize") && !inst.inline) {
								var findMax, max, maxI, i, date = new Date(2009, 12 - 1, 20), // Ensure
																								// double
																								// digits
								...
								if (dateFormat.match(/[DM]/)) {
									...
									date.setMonth(findMax(this._get(inst, (dateFormat.match(/MM/) ? "monthNames"
											: "monthNamesShort"))));
									date.setDate(findMax(this._get(inst, (dateFormat.match(/DD/) ? "dayNames"
											: "dayNamesShort")))
											+ 20 - date.getDay());
								}
						...
						_inlineDatepicker : function(target, inst) {
							...
							// If disabled option is true, disable the
							// datepicker before showing it (see ticket #5665)
							if (inst.settings.disabled) {
								this._disableDatepicker(target);
							}
							// Set display:block in place of inst.dpDiv.show()
							// which won't work on disconnected elements
							// http://bugs.jqueryui.com/ticket/7552 - A
							// Datepicker created on a detached div has zero
							// height
						 ...
						 * input element - ignored @param date string or Date -
						 * the initial date to display @param onSelect function -
						 * the function to call when a date is selected @param
						 * settings object - update the dialog date picker
						 * instance's settings (anonymous object) @param pos
						 * int[2] - coordinates for the dialog's position within
						 * the screen or event - with x/y coordinates or leave
						 * empty for default (screen centre) @return the manager
						 * object
						 */
						_dialogDatepicker : function(input, date, onSelect, settings, pos) {
							var id, browserWidth, browserHeight, scrollX, scrollY, inst = this._dialogInst; // internal
																											// instance
						...
						_destroyDatepicker : function(target) {
							...
							if (nodeName === "input") {
								inst.append.remove();
								inst.trigger.remove();
								$target.removeClass(this.markerClassName).unbind("focus", this._showDatepicker).unbind(
						...
						_optionDatepicker : function(target, name, value) {
							...
							if (arguments.length === 2 && typeof name === "string") {
								return (name === "defaults" ? $.extend({}, $.datepicker._defaults)
										: (inst ? (name === "all" ? $.extend({}, inst.settings) : this._get(inst, name))
												: null));
							}
							...
							if (inst) {
								...
								// dateFormat changes and a new minDate/maxDate
								// isn't provided
						 ...
						 * field or division or span @param noDefault boolean -
						 * true if no default date is to be used @return Date -
						 * the current date
						 */
							...
							if ($.datepicker._datepickerShowing) {
								...
								case 37:
									...
									if (event.originalEvent.altKey) {
										$.datepicker._adjustDate(event.target, (event.ctrlKey ? -$.datepicker._get(
												inst, "stepBigMonths") : -$.datepicker._get(inst, "stepMonths")), "M");
									}
						...
						_doKeyUp : function(event) {
							...
							if (inst.input.val() !== inst.lastVal) {
								try {
									date = $.datepicker
											.parseDate($.datepicker._get(inst, "dateFormat"), (inst.input ? inst.input
													.val() : null), $.datepicker._getFormatConfig(inst));
						...
						/*
						 * Pop-up the date picker for a given input field. If
						 * false returned from beforeShow event handler do not
						 * show. @param input element - the input field attached
						 * to the date picker or event - if triggered by focus
						 */
						_showDatepicker : function(input) {
							...
							inst.dpDiv.css({
								position : ($.datepicker._inDialog && $.blockUI ? "static" : (isFixed ? "fixed"
										: "absolute")),
								display : "none",
								left : offset.left + "px",
								top : offset.top + "px"
						...
						_updateDatepicker : function(inst) {
							this.maxRows = 4; // Reset the max number of rows
												// being displayed (see #7043)
							...
							// deffered render of the years select (to avoid
							// flashes on Firefox)
							if (inst.yearshtml) {
						...
						_checkOffset : function(inst, offset, isFixed) {
							var dpWidth = inst.dpDiv.outerWidth(), dpHeight = inst.dpDiv.outerHeight(), inputWidth = inst.input ? inst.input
									.outerWidth()
									: 0, inputHeight = inst.input ? inst.input.outerHeight() : 0, viewWidth = document.documentElement.clientWidth
									+ (isFixed ? 0 : $(document).scrollLeft()), viewHeight = document.documentElement.clientHeight
									+ (isFixed ? 0 : $(document).scrollTop());

							offset.left -= (this._get(inst, "isRTL") ? (dpWidth - inputWidth) : 0);
							offset.left -= (isFixed && offset.left === inst.input.offset().left) ? $(document)
									.scrollLeft() : 0;
							...
							offset.left -= Math.min(offset.left,
									(offset.left + dpWidth > viewWidth && viewWidth > dpWidth) ? Math.abs(offset.left
											+ dpWidth - viewWidth) : 0);
							offset.top -= Math.min(offset.top,
									(offset.top + dpHeight > viewHeight && viewHeight > dpHeight) ? Math.abs(dpHeight
											+ inputHeight) : 0);
						 ...
						 * the input field attached to the date picker
						 */
						_hideDatepicker : function(input) {
							...
							if (this._datepickerShowing) {
								...
								// DEPRECATED: after BC for 1.8.x $.effects[
								// showAnim ] is not needed
						...
						_selectDay : function(id, month, year, td) {
							...
							this._selectDate(id, this._formatDate(inst, inst.currentDay, inst.currentMonth,
									inst.currentYear));
						},
						...
						parseDate : function(format, value, settings) {
							...
							var iFormat, dim, extra, iValue = 0, shortYearCutoffTemp = (settings ? settings.shortYearCutoff
									: null)
									|| this._defaults.shortYearCutoff, shortYearCutoff = (typeof shortYearCutoffTemp !== "string" ? shortYearCutoffTemp
									: new Date().getFullYear() % 100 + parseInt(shortYearCutoffTemp, 10)), dayNamesShort = (settings ? settings.dayNamesShort
									: null)
									|| this._defaults.dayNamesShort, dayNames = (settings ? settings.dayNames : null)
									|| this._defaults.dayNames, monthNamesShort = (settings ? settings.monthNamesShort
									: null)
									|| this._defaults.monthNamesShort, monthNames = (settings ? settings.monthNames
									: null)
									|| this._defaults.monthNames, year = -1, month = -1, day = -1, doy = -1, literal = false, date,
							...
							getNumber = function(match) {
								var isDoubled = lookAhead(match), size = (match === "@" ? 14 : (match === "!" ? 20
										: (match === "y" && isDoubled ? 4 : (match === "o" ? 3 : 2)))), digits = new RegExp(
										"^\\d{1," + size + "}"), num = value.substring(iValue).match(digits);
								if (!num) {
						...
						_ticksTo1970 : (((1970 - 1) * 365 + Math.floor(1970 / 4) - Math.floor(1970 / 100) + Math
						 ...
						 * timestamp (ms since 01/01/1970) ! - Windows ticks
						 * (100ns since 01/01/0001) "..." - literal text '' -
						 * single quote
						 * 
						...
						formatDate : function(format, date, settings) {
							...
							var iFormat, dayNamesShort = (settings ? settings.dayNamesShort : null)
									|| this._defaults.dayNamesShort, dayNames = (settings ? settings.dayNames : null)
									|| this._defaults.dayNames, monthNamesShort = (settings ? settings.monthNamesShort
									: null)
									|| this._defaults.monthNamesShort, monthNames = (settings ? settings.monthNames
								...
								for (iFormat = 0; iFormat < format.length; iFormat++) {
									if (literal) {
										...
										case "D":
											...
											break;
										case "o":
											output += formatNumber("o", Math.round((new Date(date.getFullYear(), date
													.getMonth(), date.getDate()).getTime() - new Date(date
						...
						_setDateFromField : function(inst, noDefault) {
							...
							var dateFormat = this._get(inst, "dateFormat"), dates = inst.lastVal = inst.input ? inst.input
									.val()
									: null, defaultDate = this._getDefaultDate(inst), date = defaultDate, settings = this
						...
						_getDefaultDate : function(inst) {
							return this._restrictMinMax(inst, this._determineDate(inst, this._get(inst, "defaultDate"),
									new Date()));
						},
						...
						_determineDate : function(inst, date, defaultDate) {
							var offsetNumeric = function(offset) {
								...
								try {
									return $.datepicker.parseDate($.datepicker._get(inst, "dateFormat"), offset,
								...
								var date = (offset.toLowerCase().match(/^c/) ? $.datepicker._getDate(inst) : null)
										|| new Date(), year = date.getFullYear(), month = date.getMonth(), day = date
										.getDate(), pattern = /([+\-]?[0-9]+)\s*(d|D|w|W|m|M|y|Y)?/g, matches = pattern
										.exec(offset);
						...
						_setDate : function(inst, date, noChange) {
							var clear = !date, origMonth = inst.selectedMonth, origYear = inst.selectedYear, newDate = this
									._restrictMinMax(inst, this._determineDate(inst, date, new Date()));
						...
						_getDate : function(inst) {
							var startDate = (!inst.currentYear || (inst.input && inst.input.val() === "") ? null : this
									._daylightSavingAdjust(new Date(inst.currentYear, inst.currentMonth,
											inst.currentDay)));
							return startDate;
						},

						/*
						 * Attach the onxxx handlers. These are declared
						 * statically so they work with static code transformers
						 * like Caja.
						 */
						_attachHandlers : function(inst) {
							var stepMonths = this._get(inst, "stepMonths"), id = "#" + inst.id.replace(/\\\\/g, "\\");
							inst.dpDiv.find("[data-handler]").map(
									function() {
										var handler = {
											prev : function() {
											...
											selectDay : function() {
												$.datepicker._selectDay(id, +this.getAttribute("data-month"), +this
														.getAttribute("data-year"), this);
												return false;
											},
						...
						_generateHTML : function(inst) {
							var maxDraw, prevText, prev, nextText, next, currentText, gotoDate, controls, buttonPanel, firstDay, showWeek, dayNames, dayNamesMin, monthNames, monthNamesShort, beforeShowDay, showOtherMonths, selectOtherMonths, defaultDate, html, dow, row, group, col, selectedDate, cornerClass, calender, thead, day, daysInMonth, leadDays, curRows, numRows, printDate, dRow, tbody, daySettings, otherMonth, unselectable, tempDate = new Date(), today = this
									._daylightSavingAdjust(new Date(tempDate.getFullYear(), tempDate.getMonth(),
											tempDate.getDate())), // clear
																	// time
							isRTL = this._get(inst, "isRTL"), showButtonPanel = this._get(inst, "showButtonPanel"), hideIfNoPrevNext = this
									._get(inst, "hideIfNoPrevNext"), navigationAsDateFormat = this._get(inst,
									"navigationAsDateFormat"), numMonths = this._getNumberOfMonths(inst), showCurrentAtPos = this
									._get(inst, "showCurrentAtPos"), stepMonths = this._get(inst, "stepMonths"), isMultiMonth = (numMonths[0] !== 1 || numMonths[1] !== 1), currentDate = this
									._daylightSavingAdjust((!inst.currentDay ? new Date(9999, 9, 9) : new Date(
											inst.currentYear, inst.currentMonth, inst.currentDay))), minDate = this
									._getMinMaxDate(inst, "min"), maxDate = this._getMinMaxDate(inst, "max"), drawMonth = inst.drawMonth
									- showCurrentAtPos, drawYear = inst.drawYear;
							...
							if (maxDate) {
								maxDraw = this._daylightSavingAdjust(new Date(maxDate.getFullYear(), maxDate.getMonth()
										- (numMonths[0] * numMonths[1]) + 1, maxDate.getDate()));
								...
								for (col = 0; col < numMonths[1]; col++) {
									...
									calender += "<div class='ui-datepicker-header ui-widget-header ui-helper-clearfix"
											...
											+ this._generateMonthYearHeader(inst, drawMonth, drawYear, minDate,
													maxDate, row > 0 || col > 0, monthNames, monthNamesShort) + // draw
																												// month
																												// headers
											"</div><table class='ui-datepicker-calendar'><thead>" + "<tr>";
									thead = (showWeek ? "<th class='ui-datepicker-week-col'>"
											+ this._get(inst, "weekHeader") + "</th>" : "");
									for (dow = 0; dow < 7; dow++) { // days of
																	// the week
										day = (dow + firstDay) % 7;
										thead += "<th"
									...
									for (dRow = 0; dRow < numRows; dRow++) { // create
																				// date
																				// picker
																				// rows
										calender += "<tr>";
						...
						_generateMonthYearHeader : function(inst, drawMonth, drawYear, minDate, maxDate, secondary,
							...
							var inMinYear, inMaxYear, month, years, thisYear, determineYear, year, endYear, changeMonth = this
									._get(inst, "changeMonth"), changeYear = this._get(inst, "changeYear"), showMonthAfterYear = this
								...
								if (secondary || !changeYear) {
									...
									for (; year <= endYear; year++) {
										inst.yearshtml += "<option value='" + year + "'"
												+ (year === drawYear ? " selected='selected'" : "") + ">" + year
												+ "</option>";
									}
						...
						_adjustInstDate : function(inst, offset, period) {
							var year = inst.drawYear + (period === "Y" ? offset : 0), month = inst.drawMonth
									+ (period === "M" ? offset : 0), day = Math.min(inst.selectedDay, this
									._getDaysInMonth(year, month))
									+ (period === "D" ? offset : 0), date = this._restrictMinMax(inst, this
									._daylightSavingAdjust(new Date(year, month, day)));
						...
						_restrictMinMax : function(inst, date) {
							var minDate = this._getMinMaxDate(inst, "min"), maxDate = this._getMinMaxDate(inst, "max"), newDate = (minDate
									&& date < minDate ? minDate : date);
						...
						_notifyChange : function(inst) {
							...
							if (onChange) {
								onChange.apply((inst.input ? inst.input[0] : null), [ inst.selectedYear,
						...
						_getNumberOfMonths : function(inst) {
							...
							return (numMonths == null ? [ 1, 1 ] : (typeof numMonths === "number" ? [ 1, numMonths ]
									: numMonths));
						},
						...
						_canAdjustMonth : function(inst, offset, curYear, curMonth) {
							var numMonths = this._getNumberOfMonths(inst), date = this._daylightSavingAdjust(new Date(
									curYear, curMonth + (offset < 0 ? offset : numMonths[0] * numMonths[1]), 1));
						...
						_isInRange : function(inst, date) {
							var yearSplit, currentYear, minDate = this._getMinMaxDate(inst, "min"), maxDate = this
									._getMinMaxDate(inst, "max"), minYear = null, maxYear = null, years = this._get(
									inst, "yearRange");
							if (years) {
						...
						_getFormatConfig : function(inst) {
							...
							shortYearCutoff = (typeof shortYearCutoff !== "string" ? shortYearCutoff : new Date()
									.getFullYear()
						...
						_formatDate : function(inst, day, month, year) {
							...
							var date = (day ? (typeof day === "object" ? day : this._daylightSavingAdjust(new Date(
									year, month, day))) : this._daylightSavingAdjust(new Date(inst.currentYear,
	...
	/*
	 * Bind hover events for datepicker elements. Done via delegate so the
	 * binding only occurs once in the lifetime of the parent div. Global
	 * instActive, set by _updateDatepicker allows the handlers to find their
	...
	$.fn.datepicker = function(options) {
		...
		return this.each(function() {
			typeof options === "string" ? $.datepicker["_" + options + "Datepicker"].apply($.datepicker, [ this ]
			...
			.widget(
					"ui.dialog",
					{
						...
						_create : function() {
							...
							this.element.show().removeAttr("title").addClass("ui-dialog-content ui-widget-content")
						...
						_destroy : function() {
							...
							this.element.removeUniqueId().removeClass("ui-dialog-content ui-widget-content").css(
									this.originalCss)
						...
						close : function(event) {
							...
							if (!this.opener.filter(":focusable").focus().length) {
								// Hiding a focused element doesn't trigger blur
								// in WebKit
								// so in case we have nothing to focus on,
						...
						_focusTabbable : function() {
							...
							// 1. First element inside the dialog matching
							// [autofocus]
						...
						_createWrapper : function() {
							...
							// We assume that any existing aria-describedby
							// attribute means
							// that the dialog content is marked up properly
							// otherwise we brute force the content as the
							// description
						...
						_createTitlebar : function() {
							...
							this.uiDialogTitlebar = $("<div>").addClass(
									"ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix").prependTo(
									this.uiDialog);
							this._on(this.uiDialogTitlebar, {
								mousedown : function(event) {
									// Don't prevent click on close button
									// (#8838)
									// Focusing a dialog that is partially
									// scrolled out of view
									// causes the browser to scroll it into
									// view, preventing the click event
							...
							uiDialogTitle = $("<span>").uniqueId().addClass("ui-dialog-title").prependTo(
						...
						_createButtonPane : function() {
							...
							this.uiButtonSet = $("<div>").addClass("ui-dialog-buttonset").appendTo(
						...
						_makeResizable : function() {
							...
							// .ui-resizable has position: relative defined in
							// the stylesheet
							// but dialogs have to use absolute or fixed
							// positioning
							position = this.uiDialog.css("position"), resizeHandles = typeof handles === "string" ? handles
									: "n,e,s,w,se,sw,ne,nw";
						...
						_position : function() {
							// Need to show the dialog to get the actual offset
							// in the position plugin
						...
						_size : function() {
							...
							// .ui-dialog and .ui-dialog-content
							// divs will both have width and height set, so we
							// need to reset them
							...
							// determine the height of all the non-content
							// elements
							...
							maxContentHeight = typeof options.maxHeight === "number" ? Math.max(0, options.maxHeight
									- nonContentHeight) : "none";
						...
						_createOverlay : function() {
							...
							if (!$.ui.dialog.overlayInstances) {
								...
								// We use a delay in case the overlay is created
								// from an
									...
									if ($.ui.dialog.overlayInstances) {
										this.document.bind("focusin.dialog", function(event) {
											if (!that._allowInteraction(event)) {
												event.preventDefault();
												$(".ui-dialog:visible:last .ui-dialog-content").data(widgetFullName)
														._focusTabbable();
											}
										});
									}
			...
			_position : function() {
				...
				if (position) {
					if (typeof position === "string" || (typeof position === "object" && "0" in position)) {
						...
						position = {
							my : myAt[0] + (offset[0] < 0 ? offset[0] : "+" + offset[0]) + " " + myAt[1]
...
(function($, undefined) {
	...
	$.effects.effect.blind = function(o, done) {
		// Create element
		var el = $(this), props = [ "position", "top", "bottom", "left", "right", "height", "width" ], mode = $.effects
				.setMode(el, o.mode || "hide"), direction = o.direction || "up", vertical = rvertical.test(direction), ref = vertical ? "height"
				: "width", ref2 = vertical ? "top" : "left", motion = rpositivemotion.test(direction), animation = {}, show = mode === "show", wrapper, distance, margin;
...
(function($, undefined) {
	...
	$.effects.effect.bounce = function(o, done) {
		...
		// defaults:
		mode = $.effects.setMode(el, o.mode || "effect"), hide = mode === "hide", show = mode === "show", direction = o.direction
				|| "up", distance = o.distance, times = o.times || 5,
...
(function($, undefined) {
	...
	$.effects.effect.clip = function(o, done) {
		// Create element
		var el = $(this), props = [ "position", "top", "bottom", "left", "right", "height", "width" ], mode = $.effects
				.setMode(el, o.mode || "hide"), show = mode === "show", direction = o.direction || "vertical", vert = direction === "vertical", size = vert ? "height"
				: "width", position = vert ? "top" : "left", animation = {}, wrapper, animate, distance;
...
(function($, undefined) {
	...
	$.effects.effect.drop = function(o, done) {
		...
		var el = $(this), props = [ "position", "top", "bottom", "left", "right", "opacity", "height", "width" ], mode = $.effects
				.setMode(el, o.mode || "hide"), show = mode === "show", direction = o.direction || "left", ref = (direction === "up" || direction === "down") ? "top"
				: "left", motion = (direction === "up" || direction === "left") ? "pos" : "neg", animation = {
			opacity : show ? 1 : 0
		}, distance;
...
(function($, undefined) {
	...
	$.effects.effect.explode = function(o, done) {
		...
		var rows = o.pieces ? Math.round(Math.sqrt(o.pieces)) : 3, cells = rows, el = $(this), mode = $.effects
				.setMode(el, o.mode || "hide"), show = mode === "show",
...
(function($, undefined) {
	...
	$.effects.effect.fold = function(o, done) {
		...
		// Create element
		var el = $(this), props = [ "position", "top", "bottom", "left", "right", "height", "width" ], mode = $.effects
				.setMode(el, o.mode || "hide"), show = mode === "show", hide = mode === "hide", size = o.size || 15, percent = /([0-9]+)%/
				.exec(size), horizFirst = !!o.horizFirst, widthFirst = show !== horizFirst, ref = widthFirst ? [
				"width", "height" ] : [ "height", "width" ], duration = o.duration / 2, wrapper, distance, animation1 = {}, animation2 = {};
...
(function($, undefined) {
	...
	$.effects.effect.pulsate = function(o, done) {
		...
		anims = ((o.times || 5) * 2) + (showhide ? 1 : 0), duration = o.duration / anims, animateTo = 0, queue = elem
				.queue(), queuelen = queue.length, i;
...
(function($, undefined) {
	...
	$.effects.effect.puff = function(o, done) {
		var elem = $(this), mode = $.effects.setMode(elem, o.mode || "hide"), hide = mode === "hide", percent = parseInt(
				o.percent, 10) || 150, factor = percent / 100, original = {
			height : elem.height(),
			width : elem.width(),
	...
	$.effects.effect.scale = function(o, done) {
		...
		// Create element
		var el = $(this), options = $.extend(true, {}, o), mode = $.effects.setMode(el, o.mode || "effect"), percent = parseInt(
				o.percent, 10)
	...
	$.effects.effect.size = function(o, done) {
		...
		// Create element
		var original, baseline, factor, el = $(this), props0 = [ "position", "top", "bottom", "left", "right", "width",
		...
		// Copy for children
		props2 = [ "width", "height", "overflow" ], cProps = [ "fontSize" ], vProps = [ "borderTopWidth",
				"borderBottomWidth", "paddingTop", "paddingBottom" ], hProps = [ "borderLeftWidth", "borderRightWidth",
				"paddingLeft", "paddingRight" ],

		// Set options
		mode = $.effects.setMode(el, o.mode || "effect"), restore = o.restore || mode !== "effect", scale = o.scale
				|| "both", origin = o.origin || [ "middle", "center" ], position = el.css("position"), props = restore ? props0
				: props1, zero = {
			height : 0,
			width : 0,
			outerHeight : 0,
...
(function($, undefined) {
	...
	$.effects.effect.shake = function(o, done) {
		...
		var el = $(this), props = [ "position", "top", "bottom", "left", "right", "height", "width" ], mode = $.effects
				.setMode(el, o.mode || "effect"), direction = o.direction || "left", distance = o.distance || 20, times = o.times || 3, anims = times * 2 + 1, speed = Math
				.round(o.duration / anims), ref = (direction === "up" || direction === "down") ? "top" : "left", positiveMotion = (direction === "up" || direction === "left"), animation = {}, animation1 = {}, animation2 = {}, i,
...
(function($, undefined) {
	...
	$.effects.effect.slide = function(o, done) {
		...
		// Create element
		var el = $(this), props = [ "position", "top", "bottom", "left", "right", "width", "height" ], mode = $.effects
				.setMode(el, o.mode || "show"), show = mode === "show", direction = o.direction || "left", ref = (direction === "up" || direction === "down") ? "top"
				: "left", positiveMotion = (direction === "up" || direction === "left"), distance, animation = {};
...
(function($, undefined) {
	...
	$.effects.effect.transfer = function(o, done) {
		var elem = $(this), target = $(o.to), targetFixed = target.css("position") === "fixed", body = $("body"), fixTop = targetFixed ? body
				.scrollTop()
				: 0, fixLeft = targetFixed ? body.scrollLeft() : 0, endPosition = target.offset(), animation = {
			top : endPosition.top - fixTop,
			left : endPosition.left - fixLeft,
			height : target.innerHeight(),
			width : target.innerWidth()
		}, startPosition = elem.offset(), transfer = $("<div class='ui-effects-transfer'></div>").appendTo(
				document.body).addClass(o.className).css({
...
(function($, undefined) {
	...
	$.widget("ui.menu", {
		...
		_create : function() {
			...
			this.element.uniqueId().addClass("ui-menu ui-widget ui-widget-content ui-corner-all").toggleClass(
		...
		_destroy : function() {
			...
			this.element.removeAttr("aria-activedescendant").find(".ui-menu").addBack().removeClass(
					"ui-menu ui-widget ui-widget-content ui-corner-all ui-menu-icons").removeAttr("role").removeAttr(
					"tabIndex").removeAttr("aria-labelledby").removeAttr("aria-expanded").removeAttr("aria-hidden")
					.removeAttr("aria-disabled").removeUniqueId().show();

			// Destroy menu items
			this.element.find(".ui-menu-item").removeClass("ui-menu-item").removeAttr("role").removeAttr(
					"aria-disabled").children("a").removeUniqueId().removeClass("ui-corner-all ui-state-hover")
					.removeAttr("tabIndex").removeAttr("role").removeAttr("aria-haspopup").children().each(function() {
						var elem = $(this);
		...
		refresh : function() {
			...
			submenus.filter(":not(.ui-menu)").addClass("ui-menu ui-widget ui-widget-content ui-corner-all").hide()
					.attr({
						role : this.options.role,
						"aria-hidden" : "true",
						"aria-expanded" : "false"
							...
							function() {
								var menu = $(this), item = menu.prev("a"), submenuCarat = $("<span>").addClass(
										"ui-menu-icon ui-icon " + icon).data("ui-menu-submenu-carat", true);
			...
			menus.children(":not(.ui-menu-item):has(a)").addClass("ui-menu-item").attr("role", "presentation")
					.children("a").uniqueId().addClass("ui-corner-all").attr({
						tabIndex : -1,
						role : this._itemRole()
					});

			// Initialize unlinked menu-items containing spaces and/or dashes
			// only as dividers
		...
		_close : function(startMenu) {
			...
			startMenu.find(".ui-menu").hide().attr("aria-hidden", "true").attr("aria-expanded", "false").end().find(
...
(function($, undefined) {
	...
	function getOffsets(offsets, width, height) {
		return [ parseFloat(offsets[0]) * (rpercent.test(offsets[0]) ? width / 100 : 1),
	...
	$.position = {
		scrollbarWidth : function() {
			...
			var w1, w2, div = $("<div style='display:block;width:50px;height:50px;overflow:hidden;'><div style='height:100px;width:auto;'></div></div>"), innerDiv = div
					.children()[0];
		...
		getScrollInfo : function(within) {
			var overflowX = within.isWindow ? "" : within.element.css("overflow-x"), overflowY = within.isWindow ? ""
	...
	$.fn.position = function(options) {
		...
		var atOffset, targetWidth, targetHeight, targetOffset, basePosition, dimensions, target = $(options.of), within = $.position
				.getWithinInfo(options.within), scrollInfo = $.position.getScrollInfo(within), collision = (options.collision || "flip")
				.split(" "), offsets = {};
		...
		$.each([ "my", "at" ], function() {
			...
			if (pos.length === 1) {
				pos = rhorizontal.test(pos[0]) ? pos.concat([ "center" ]) : rvertical.test(pos[0]) ? [ "center" ]
		...
		return this
				.each(function() {
					var collisionPosition, using, elem = $(this), elemWidth = elem.outerWidth(), elemHeight = elem
							.outerHeight(), marginLeft = parseCss(this, "marginLeft"), marginTop = parseCss(this,
							"marginTop"), collisionWidth = elemWidth + marginLeft + parseCss(this, "marginRight")
							+ scrollInfo.width, collisionHeight = elemHeight + marginTop
							+ parseCss(this, "marginBottom") + scrollInfo.height, position = $.extend({}, basePosition), myOffset = getOffsets(
							offsets.my, elem.outerWidth(), elem.outerHeight());
					...
					if (options.using) {
						...
						using = function(props) {
							var left = targetOffset.left - position.left, right = left + targetWidth - elemWidth, top = targetOffset.top
									- position.top, bottom = top + targetHeight - elemHeight, feedback = {
								target : {
									element : target,
	...
	$.ui.position = {
		fit : {
			left : function(position, data) {
				var within = data.within, withinOffset = within.isWindow ? within.scrollLeft : within.offset.left, outerWidth = within.width, collisionPosLeft = position.left
						- data.collisionPosition.marginLeft, overLeft = withinOffset - collisionPosLeft, overRight = collisionPosLeft
			...
			top : function(position, data) {
				var within = data.within, withinOffset = within.isWindow ? within.scrollTop : within.offset.top, outerHeight = data.within.height, collisionPosTop = position.top
						- data.collisionPosition.marginTop, overTop = withinOffset - collisionPosTop, overBottom = collisionPosTop
		...
		flip : {
			left : function(position, data) {
				var within = data.within, withinOffset = within.offset.left + within.scrollLeft, outerWidth = within.width, offsetLeft = within.isWindow ? within.scrollLeft
						: within.offset.left, collisionPosLeft = position.left - data.collisionPosition.marginLeft, overLeft = collisionPosLeft
						- offsetLeft, overRight = collisionPosLeft + data.collisionWidth - outerWidth - offsetLeft, myOffset = data.my[0] === "left" ? -data.elemWidth
						: data.my[0] === "right" ? data.elemWidth : 0, atOffset = data.at[0] === "left" ? data.targetWidth
						: data.at[0] === "right" ? -data.targetWidth : 0, offset = -2 * data.offset[0], newOverRight, newOverLeft;

				if (overLeft < 0) {
					newOverRight = position.left + myOffset + atOffset + offset + data.collisionWidth - outerWidth
							- withinOffset;
					if (newOverRight < 0 || newOverRight < abs(overLeft)) {
						position.left += myOffset + atOffset + offset;
					}
				} else if (overRight > 0) {
					newOverLeft = position.left - data.collisionPosition.marginLeft + myOffset + atOffset + offset
							- offsetLeft;
			...
			top : function(position, data) {
				var within = data.within, withinOffset = within.offset.top + within.scrollTop, outerHeight = within.height, offsetTop = within.isWindow ? within.scrollTop
						: within.offset.top, collisionPosTop = position.top - data.collisionPosition.marginTop, overTop = collisionPosTop
						- offsetTop, overBottom = collisionPosTop + data.collisionHeight - outerHeight - offsetTop, top = data.my[1] === "top", myOffset = top ? -data.elemHeight
						: data.my[1] === "bottom" ? data.elemHeight : 0, atOffset = data.at[1] === "top" ? data.targetHeight
						: data.at[1] === "bottom" ? -data.targetHeight : 0, offset = -2 * data.offset[1], newOverTop, newOverBottom;
				if (overTop < 0) {
					newOverBottom = position.top + myOffset + atOffset + offset + data.collisionHeight - outerHeight
							- withinOffset;
					...
					newOverTop = position.top - data.collisionPosition.marginTop + myOffset + atOffset + offset
							- offsetTop;
	...
	(function() {
		var testElement, testElementParent, testElementStyle, offsetLeft, i, body = document
				.getElementsByTagName("body")[0], div = document.createElement("div");
...
(function($, undefined) {
	...
	$.widget("ui.progressbar", {
		...
		_destroy : function() {
			this.element.removeClass("ui-progressbar ui-widget ui-widget-content ui-corner-all").removeAttr("role")
					.removeAttr("aria-valuemin").removeAttr("aria-valuemax").removeAttr("aria-valuenow");
			...
			.widget(
					...
					$.ui.mouse,
					{
						...
						_createHandles : function() {
							var i, handleCount, options = this.options, existingHandles = this.element.find(
									".ui-slider-handle").addClass("ui-state-default ui-corner-all"), handle = "<a class='ui-slider-handle ui-state-default ui-corner-all' href='#'></a>", handles = [];
						...
						_createRange : function() {
							...
							if (options.range) {
								...
								if (!this.range || !this.range.length) {
									...
									// note: this isn't the most fittingly
									// semantic framework class for this
									// element,
									// but worked best visually with a variety
									// of themes
								...
								this.range.addClass(classes
										+ ((options.range === "min" || options.range === "max") ? " ui-slider-range-"
												+ options.range : ""));
							} else {
								this.range = $([]);
							}
						...
						_destroy : function() {
							...
							this.element.removeClass("ui-slider" + " ui-slider-horizontal" + " ui-slider-vertical"
						...
						_mouseCapture : function(event) {
							...
							this.handles.each(function(i) {
								...
								if ((distance > thisDistance)
										|| (distance === thisDistance && (i === that._lastChangedValue || that
												.values(i) === o.min))) {
						...
						_change : function(event, index) {
							if (!this._keySliding && !this._mouseSliding) {
								...
								// store the last changed value index for
								// reference when handles overlap
						...
						// _values() returns array of values trimmed by min and
						// max, aligned by step
						// _values( index ) returns single value trimmed by min
						...
						// returns the step-aligned value that val is closest
						// to, between (inclusive) min and max
						_trimAlignValue : function(val) {
							...
							var step = (this.options.step > 0) ? this.options.step : 1, valModStep = (val - this
									._valueMin())
							...
							// Since JavaScript has problems with large floats,
							// round
						...
						_refreshValue : function() {
							...
							if (this.options.values && this.options.values.length) {
								...
								valPercent = (valueMax !== valueMin) ? (value - valueMin) / (valueMax - valueMin) * 100
										: 0;
						...
						_handleEvents : {
							keydown : function(event) {
								...
								var allowed, curVal, newVal, step, index = $(event.target).data(
...
(function($) {
	...
	$.widget("ui.spinner", {
		...
		_draw : function() {
			var uiSpinner = this.uiSpinner = this.element.addClass("ui-spinner-input").attr("autocomplete", "off")
					.wrap(this._uiSpinnerHtml()).parent()
					// add buttons
			...
			// button bindings
			this.buttons = uiSpinner.find(".ui-spinner-button").attr("tabIndex", -1).button().removeClass(
					"ui-corner-all");
		...
		_buttonHtml : function() {
			return "" + "<a class='ui-spinner-button ui-spinner-up ui-corner-tr'>" + "<span class='ui-icon "
					...
					+ "<a class='ui-spinner-button ui-spinner-down ui-corner-br'>" + "<span class='ui-icon "
		...
		_increment : function(i) {
			...
			if (incremental) {
				return $.isFunction(incremental) ? incremental(i) : Math.floor(i * i * i / 50000 - i * i / 500 + 17 * i
						/ 200 + 1);
			}
		...
		_parse : function(val) {
			if (typeof val === "string" && val !== "") {
				val = window.Globalize && this.options.numberFormat ? Globalize.parseFloat(val, 10,
		...
		_format : function(value) {
			...
			return window.Globalize && this.options.numberFormat ? Globalize.format(value, this.options.numberFormat,
		...
		_destroy : function() {
			this.element.removeClass("ui-spinner-input").prop("disabled", false).removeAttr("autocomplete").removeAttr(
					"role").removeAttr("aria-valuemin").removeAttr("aria-valuemax").removeAttr("aria-valuenow");
...
(function($, undefined) {
	...
	function isLocal(anchor) {
		return anchor.hash.length > 1
				&& decodeURIComponent(anchor.href.replace(rhash, "")) === decodeURIComponent(location.href.replace(
						rhash, ""));
	}
			...
			.widget(
					"ui.tabs",
					{
						...
						_create : function() {
							...
							this.element.addClass("ui-tabs ui-widget ui-widget-content ui-corner-all").toggleClass(
							...
							if ($.isArray(options.disabled)) {
								options.disabled = $.unique(
										options.disabled.concat($.map(this.tabs.filter(".ui-state-disabled"), function(
												li) {
											return that.tabs.index(li);
										}))).sort();
							}

							// check for length avoids error when initializing
							// empty list
						...
						_initialActive : function() {
							var active = this.options.active, collapsible = this.options.collapsible, locationHash = location.hash
									.substring(1);
						...
						_tabKeydown : function(event) {
							/* jshint maxcomplexity:15 */
							var focusedTab = $(this.document[0].activeElement).closest("li"), selectedIndex = this.tabs
									.index(focusedTab), goingForward = true;
							...
							// Focus the appropriate tab, based on which key was
							// pressed
							event.preventDefault();
							clearTimeout(this.activating);
							selectedIndex = this._focusNextTab(selectedIndex, goingForward);

							// Navigating with control key will prevent
							// automatic activation
							if (!event.ctrlKey) {
								// Update aria-selected immediately so that AT
								// think the tab is already selected.
								// Otherwise AT may confuse the user by stating
								// that they need to activate the tab,
								// but the tab will already be activated by the
								// time the announcement finishes.
						...
						// Alt+page up/down moves focus to the previous/next tab
						// (and activates)
						...
						_setOption : function(key, value) {
							...
							if (key === "disabled") {
								// don't use the widget factory's disabled
								// handling
							...
							if (key === "collapsible") {
								...
								// Setting collapsible: false while collapsed;
								// open first panel
						...
						_processTabs : function() {
							...
							this.anchors.each(function(i, anchor) {
								var selector, panel, panelId, anchorId = $(anchor).uniqueId().attr("id"), tab = $(
						...
						// allow overriding how to find the list for rare usage
						// scenarios (#7715)
						_getList : function() {
						...
						_createPanel : function(id) {
							return $("<div>").attr("id", id).addClass(
									"ui-tabs-panel ui-widget-content ui-corner-bottom").data("ui-tabs-destroy", true);
						},
						...
						_eventHandler : function(event) {
							var options = this.options, active = this.active, anchor = $(event.currentTarget), tab = anchor
									.closest("li"), clickedIsActive = tab[0] === active[0], collapsing = clickedIsActive
									&& options.collapsible, toShow = collapsing ? $() : this._getPanelForTab(tab), toHide = !active.length ? $()
									: this._getPanelForTab(active), eventData = {
								oldTab : active,
								oldPanel : toHide,
						...
						_toggle : function(event, eventData) {
							...
							// start out by hiding, then showing, then
							// completing
							...
							// If we're switching tabs, remove the old tab from
							// the tab order.
							// If we're opening from collapsed state, remove the
							// previous tab from the tab order.
							// If we're collapsing, then keep the collapsing tab
							// in the tab order.
							if (toShow.length && toHide.length) {
						...
						_getIndex : function(index) {
							// meta-function to give users option to provide a
							// href string instead of a numerical index.
						...
						_destroy : function() {
							...
							this.anchors.removeClass("ui-tabs-anchor").removeAttr("role").removeAttr("tabIndex")
									.removeUniqueId();
												...
												if ($.data(this, "ui-tabs-destroy")) {
													...
													$(this)
															...
															.removeAttr("tabIndex").removeAttr("aria-live").removeAttr(
																	"aria-busy").removeAttr("aria-selected")
															.removeAttr("aria-labelledby").removeAttr("aria-hidden")
															.removeAttr("aria-expanded").removeAttr("role");
												}
											});
						...
						load : function(index, event) {
							index = this._getIndex(index);
							var that = this, tab = this.tabs.eq(index), anchor = tab.find(".ui-tabs-anchor"), panel = this
									._getPanelForTab(tab), eventData = {
								tab : tab,
								panel : panel
							};
							...
							// support: jQuery <1.8
							// jQuery <1.8 returns false if the request is
							// canceled in beforeSend,
							// but as of 1.8, $.ajax() always returns a jqXHR
							// object.
...
(function($) {
	...
	function removeDescribedBy(elem) {
		var id = elem.data("ui-tooltip-id"), describedby = (elem.attr("aria-describedby") || "").split(/\s+/), index = $
				.inArray(id, describedby);
		if (index !== -1) {
	...
	$.widget("ui.tooltip", {
		...
		_updateContent : function(target, event) {
			...
			content = contentOption.call(target[0], function(response) {
				...
				// delay this call to _open so the other call to _open runs
				// first
				that._delay(function() {
					// jQuery creates a special event for focusin when it
					// doesn't
					// exist natively. To improve performance, the native event
					// object is reused and the type is changed. Therefore, we
					// can't
		...
		_destroy : function() {
			...
			$.each(this.tooltips, function(id, element) {
				...
				// Remove immediately; destroying an open tooltip doesn't use
				// the
				// hide animation
				$("#" + id).remove();
